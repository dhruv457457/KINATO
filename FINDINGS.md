...
## 21. The faster model was 2.4× quicker, 4× cheaper, and sold 18% less

Two live calls ended with the customer hanging up. Not an error — Twilio
recorded `completed`, no error code, no rejected TwiML. The cause was
**silence**: on the turn where the agent commits the sale, the line goes
completely dead while the model thinks.

```
15:05:26.4  customer: "link again"
15:05:27.2  LLM #1          0.9s
15:05:27.6  check_offer     0.4s
15:05:30.2  LLM #2          2.4s
15:05:33.1  Razorpay link   1.2s
15:05:33.6  reply                  7.2s of nothing
```

Seven seconds of dead air on a phone is indistinguishable from a dropped
call, so the customer hung up. Both cut-off calls died on their slowest turn.

A faster model looked like the obvious fix. It measured well:

```
                              latency   calls a tool
openai/gpt-oss-120b            567 ms       3/3
meta-llama/llama-3.3-70b       952 ms       3/3
google/gemini-2.5-flash       1037 ms       3/3
openai/gpt-4o-mini  (current) 1360 ms       3/3
google/gemini-2.5-flash-lite  2613 ms       0/3
```

2.4× faster than what was running, and four times cheaper. Then the
scoreboard:

```
                  gpt-4o-mini              gpt-oss-120b
matched           25/25, 24/25, 24/25      24/25, 24/25, 21/25
rule_breaks       0, 0, 0                  0, 0, 0
recovered full    6, 7, 7                  5, 5, 2
no sale           2, 2, 2                  3, 3, 6
REVENUE           15,711 / 15,861 / 15,861  13,012 / 14,312 / 10,315
```

**Median revenue fell 18%, and the variance exploded.** Three runs of
gpt-4o-mini spread ₹150; three runs of the replacement spread ₹4,000. It was
not merely worse, it was less *predictable* — which is harder to notice,
because any single run can look acceptable.

Not shipped. Seven seconds of dead air is a cheaper problem than an agent
that recovers less money less reliably.

### Three things the latency table could not see

**`gemini-2.5-flash-lite` never called a tool.** Cheapest of the lot,
marketed as fast, 0/3 on the one behaviour the entire product depends on. It
would have talked pleasantly to every customer and sent zero links — FINDINGS
#14 again, and it would have looked like a sensible swap right up until the
recovery rate hit zero.

**Twenty providers serve `gpt-oss-120b` and several do not support tool
calling.** Routing for speed alone could land on one. It does not, because
OpenRouter filters by capability when the request carries `tools` — but that
is a guarantee borrowed from a third party, not one this codebase enforces.

**The first benchmark said the swap was pointless.** 1458ms vs 1418ms —
no gain. It used a three-token prompt. The real turn carries a full
conversation and six tool schemas, and on that shape the gap is 983ms vs
495ms. *A benchmark that does not resemble the work measures nothing*, and
this one was very nearly used to abandon the experiment before it started.

### The near-miss worth recording

Running the candidate three times overwrote `batch_recovery_report.json` —
the committed evidence file — with the rejected model's **worst** run. Two
full-price recoveries instead of seven.

Nothing caught it. It was found by asking "did you commit that?" and looking.
Had it shipped, the project's headline evidence would have described a model
it does not run, on its worst sample, while the prose around it claimed the
opposite. That is FINDINGS #6 exactly: **the measurement drifting away from
the thing it measures**, arriving this time not through a code change but
through the ordinary act of testing.

The report is generated by a script and committed as an artifact. Anything
with both properties can be silently rewritten by an experiment, and there is
no test that fails when it is.
