...
- **The sale comes first** → a customer ready to buy gets a full-price link. The agent never volunteers a discount, and a declined card is treated as a broken checkout, not a price objection — enforced by a `REJECTED_FULL_PRICE_FIRST` refusal from the deterministic failure classifier, not by a prompt.
